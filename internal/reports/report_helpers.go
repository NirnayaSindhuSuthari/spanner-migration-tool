		...
		for _, colName := range colNames {
			...
			for _, i := range issues[colId] {
				...
				case internal.SpConfiguration:
					toAppend := Issue{
						Category:    IssueDB[i].Category,
						Description: fmt.Sprintf("%s", IssueDB[i].Brief),
					}
					l = append(l, toAppend)
...
var IssueDB = map[internal.SchemaIssue]struct {
	...
	internal.SpConfiguration: 		{Brief: "Please configure spanner project id and instance id to proceed otherwise default values will not be migrated", Severity: warning, Category: "SPANNER_CONFIGURATION_WARNING"},
