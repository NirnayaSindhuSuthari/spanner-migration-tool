  ...
  getColumnMapping(tableId: string, data: IConv): IColumnTabData[] {
    ...
    const res: IColumnTabData[] = data.SrcSchema[tableId].ColIds.map((colId: string, i: number) => {
      ...
      return {
        ...
        srcDefaultValue: data.SrcSchema[tableId].ColDefs[colId].Default.DefaultValue,
        // spDefaultValue: spannerColDef && spannerColDef.Default.IsSpannerSupported ? spannerColDef.Default.DefaultValue : '',
        spDefaultValue: spannerColDef && spannerColDef.Default.IsPresent ? spannerColDef.Default.DefaultValue : '',
        srcId: colId,
      ...
      spColIds.forEach((colId: string, i: number) => {
        if (srcColIds.indexOf(colId) < 0) {
          ...
          res.push({
            ...
            srcIsNotNull: false,
            srcDefaultValue: '',
            spDefaultValue: spColumn.Default.DefaultValue,
            srcId: '',
            spId: colId,
            srcColMaxLength: '',
