...
func ReadSpannerSchema(SpProjectId string, SpInstanceId string, ctx context.Context, conv *internal.Conv, client *sp.Client) error {
	infoSchema := spanner.InfoSchemaImpl{Client: client, Ctx: ctx, SpDialect: conv.SpDialect}
	processSchema := common.ProcessSchemaImpl{}
	err := processSchema.ProcessSchema(SpProjectId, SpInstanceId, conv, infoSchema, common.DefaultWorkers, internal.AdditionalSchemaAttributes{IsSharded: false}, &common.SchemaToSpannerImpl{}, &common.UtilsOrderImpl{}, &common.InfoSchemaImpl{})
	if err != nil {
